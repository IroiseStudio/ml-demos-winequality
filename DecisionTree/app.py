...
def train_dt(
    ...
    """Train a DecisionTree (regressor or classifier) and return model, metrics, importances, feature list, and test preview."""
    df = load_wine_data()
    X, y = split_features_target(df)

    # For classification, cast y to int classes; for regression keep numeric
    ...
    if task_type == "Classification":
        ...
        model = DecisionTreeRegressor(
            max_depth=max_depth if max_depth > 0 else None,
            min_samples_split=min_samples_split,
            criterion=criterion,  # squared_error | friedman_mse | absolute_error | poisson
            random_state=random_state,
        )
    ...
    if task_type == "Classification":
        ...
        # You can also compute 'weighted' if you want both; keeping it simple for now
        metrics = {
            "Accuracy": accuracy,
        ...
        metrics = {"MAE": mae, "RMSE": rmse, "R2": r2}  # <- use 'R2' key
        metrics_table = pd.DataFrame(
    ...
    return (
        model,                  # 1
        metrics,                # 2  (dict)
        importances,            # 3  (DataFrame)
        X.columns.tolist(),     # 4  feature names
        y_test.values,          # 6  for CM
        preds,                  # 7  for CM
        metrics_table           # 8  (DataFrame)
    )
...
def on_train(
    ...
    (model, metrics, importances, feat_list, 
     y_test_arr, preds_arr, metrics_table) = train_dt(
        max_depth=max_depth,
        min_samples_split=min_samples_split,
        test_size=test_size,
        random_state=random_state,
    ...
    if task_type == "Classification":
        ...
        cm_note = ""  # nothing to say when available
    else:
        metrics_md = f"**MAE:** {metrics['MAE']:.3f} | **RMSE:** {metrics['RMSE']:.3f} | **R²:** {metrics['R2']:.3f}"
        # For regression, show a note instead of CM
    ...
    tree_plot = plot_tree_fig(model, feat_list, plot_max_depth)

    # Return in the order you wire in the UI
    return (
        metrics_md,     # metrics_out
        metrics_table,
        fi_plot,        # fi_plot
        cm_plot,        # cm_plot
        cm_note,        # cm_note (empty or explanation)
        tree_plot,      # tree_plot
        _state
    )
...
def build_ui() -> gr.Blocks:
    with gr.Blocks(title="Wine Quality Dataset — Decision Tree") as demo:
        gr.Markdown(
            """
            # Wine Quality Dataset — Decision Tree

            This interactive demo trains and visualizes a **Decision Tree** model on the combined [red and white wine dataset from UCI](https://archive.ics.uci.edu/dataset/186/wine+quality).  
            ...
            **Tips**
            - The full tree can become very large. Adjust **Plot max depth** to keep it readable.  
            - *Model hyperparameters* affect the learned tree, while *training config* controls the dataset split and randomness.  
            """
        )
        ...
        with gr.Tab("Train/Evaluate"):
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    gr.Markdown("### Task")
                    task_type = gr.Radio(choices=list(TASK_MODES.keys()), value="Regression", label="Choose task")
                    task_help = gr.Markdown(TASK_MODES["Regression"]["blurb"])
            ...
            with gr.Tabs():
                ...
                with gr.TabItem("Tree Plot"):
                    tree_plot = gr.Plot(label="Decision Tree plot")          # 7

            train_btn.click(
                on_train,
                inputs=[max_depth, min_samples_split, test_size, random_state, plot_max_depth, task_type, criterion, state],
